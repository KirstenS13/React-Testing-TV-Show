...
test('shows fetching data message and calls api', async () => {
  //call mocked api call, return mock data
  mockFetchShow.mockResolvedValueOnce(data);
  //render app and get queries
  const {getByText, findByDisplayValue, findByText, findAllByTestId} = render(<App />);

  //test that the loading message is there before the api call is done
  const fetchingMessage = getByText(/fetching data.../i);
  expect(fetchingMessage).toBeVisible();

  //give the api time to return the data
  setTimeout(() => {
    //click on the dropdown menu
    const dropdown = findByDisplayValue(/select a season/i);
    ...
    //confirm that the episodes rendered to the page
    const episodes = findAllByTestId(/episode/i);

    waitFor(() => {
      //check that api was called and that dropdown and options were visible
